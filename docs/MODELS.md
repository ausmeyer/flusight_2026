...
- Gaussian response: log1p(y[t+h]) − log1p(y[t]); direct pooled horizons h=1..4. Integration uses only the observed anchor value.
...
ED forecasts are modeled in percentage points and divided by 100 at export, bounded to [0,1]. The hub's written plausibility limit of 0.25 is checked and triggers a failure if exceeded; forecasts are not silently truncated to that plausibility threshold. Hospitalization forecasts are integer-valued, nonnegative and checked against the written 30%-of-population bound.
